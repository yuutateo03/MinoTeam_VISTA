...
from .write_csv_skeletons import create_csv_skeletons, CSVWriter
...
def generate_csv():
    ...
    logger.info("Starting fine-tuned VisDrone YOLO model and ByteTrack tracker...")

    # Define operational threshold for drone-view small object retention
    YOLO_CONF_THRESHOLD = 0.05

    pipeline = LightweightPipelineLocate(
        yolo_model=YOLO("yolo26x_visdrone.pt"),
    ...
    for result in pipeline.process_video(video_path):
        ...
        for det in result.detections:
            # Match the pipeline filter threshold so small aerial objects are not dropped
            if det.confidence >= YOLO_CONF_THRESHOLD:
    ...
    # 5. Save Final Track Captions (Captioning Finale)
